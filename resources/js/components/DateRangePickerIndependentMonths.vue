...
const locale = ref('es-VE');
const formatter = useDateFormatter(locale.value);

const placeholder = ref(today(getLocalTimeZone())) as Ref<DateValue>;
const secondMonthPlaceholder = ref(today(getLocalTimeZone()).add({ months: 1 })) as Ref<DateValue>;
...
<template>
  <Popover>
    <PopoverTrigger as-child>
      <Button id="date_range" variant="outline" :class="cn('w-[280px] justify-start text-left font-normal', !model && 'text-muted-foreground')">
        ...
        <template v-else> Escoja un rango de fechas </template>
      </Button>
    </PopoverTrigger>
    <PopoverContent class="w-auto p-0">
      <RangeCalendarRoot locale="es-VE" v-slot="{ weekDays }" v-model="model" v-model:placeholder="placeholder" class="p-3">
